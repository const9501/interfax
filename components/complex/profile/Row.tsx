import Link from "next/link";
import {useAppDispatch} from "../../../hooks/useAppDispatch";
import {fetchCommits, goToInitialState} from "../../../store/commits/commitsSlice";
import {FC} from "react";
import {IRepo} from "../../../store/repos/types";
import styles from "./Row.module.scss";


interface IRowProps {
  repo: IRepo
  index: number
}

const Row: FC<IRowProps> = ({repo, index}) => {

  const dispatch = useAppDispatch()

    return (
      <div className={index%2 === 0 ? styles.evenRow : styles.oddRow}>

        <div
          className={styles.name}
          onClick={() => {
            dispatch(goToInitialState())
            dispatch(fetchCommits(repo.commits_url.split('{')[0]))
          }}
        >
          {repo.name && <Link href={'/profile/repo'}>{repo.name}</Link>}
        </div>

        <div className='col-span-2'>{repo.language}</div>

        <div className='col-span-4'>{repo.description}</div>

        <div className={styles.stars}>
          <svg aria-hidden="true" className="w-5 h-5 text-yellow-400" fill="currentColor" viewBox="0 0 20 20"
               xmlns="http://www.w3.org/2000/svg"><title>Rating star</title>
            <path
              d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path>
          </svg>
          <p className={styles.count}>{repo.stargazers_count}</p>
        </div>

      </div>
    );

}

export default Row;